...
def build_extrinsic_matrix(camera_row: int, camera_col: int, baseline: float) -> np.ndarray:
    
    # Construire à partir d'une matrice identité
    extrinsic_matrix = np.identity(4)

    # Camera translation in X
    extrinsic_matrix[0, 3] = baseline * camera_row

    # Camera translation in Y
    extrinsic_matrix[1, 3] = baseline * camera_col
...
def build_intrinsic_matrix(image_width, image_height, fovy) -> np.ndarray:
    ...
    captor_height = image_height
    captor_width = image_width
    ...
    captor_height = 1
    captor_width = captor_height * (image_width / image_height)
    '''

    # On peut déduire la distance focale d'après les autres paramètres
    focal_length = deduce_focal_length(captor_height, fovy)

    '''
    # En définissant d'abord focal_length
    focal_length = 0.035
    captor_height = deduce_captor_height(focal_length, fovy)
    captor_width = captor_height * (image_width / image_height)
    '''
    ...
    intrinsic_matrix[0, 2] = image_width / 2
    intrinsic_matrix[1, 2] = image_height / 2
...
#
def build_cams_dir(
    output_dir: str, intrinsic_matrix: np.ndarray,
    pos_cameras: np.ndarray, depth_min: float,
    depth_interval: float, baseline: float) -> None:
    ...
    for camera_id in range(config.num_cameras):
        ...
        extrinsic_matrix = build_extrinsic_matrix(camera_row, camera_col, baseline)
