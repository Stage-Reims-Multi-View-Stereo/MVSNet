...
def save_as_mvsnet_pairs_file(best_scores, best_indices, output_file_name: str) -> None:
    ...
    for camera_index in range(num_cameras):
        ...
        current_num_neighbours = np.count_nonzero(
            best_scores[camera_index] >= config.min_neighbour_score)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Générer le fichier de caméra pair.txt ou le répertoire cams/ pour MVSNet.")
    parser.add_argument("output_pairs_dir", type=str, help="Créer le fichier pair.txt dans ce répertoire")
    parser.add_argument("--plot", action=argparse.BooleanOptionalAction, help="Afficher la disposition dans une fenêtre")
    parser.add_argument("--print", action=argparse.BooleanOptionalAction, help="Afficher l'arrangement des caméras dans la console.")
    
    if len(sys.argv) == 1:
        # Afficher l'aide s'il n'y a aucun argument
        # (Sinon le script ne ferait rien mais il n'y aura aucune erreur, mais ce n'est pas très indicatif pour l'utilisateur)
        parser.print_help()
    ...
    best_scores, best_indices = compute_best_scores(pos_cameras, num_bests)

    if args.print:
        print("Indices des caméras:")
        print(indices_cameras)

        print("Position des caméras:")
        print(pos_cameras)

        print(f'Extrait les {num_bests} meilleures voisins.')
        print('pos_cameras:\n', pos_cameras)
    ...
    if args.plot:
        plot_arangement(pos_cameras, best_scores, best_indices)
    
    os.makedirs(args.output_pairs_dir, exist_ok=True)
    save_as_mvsnet_pairs_file(best_scores, best_indices, os.path.join(args.output_pairs_dir, "pair.txt"))
